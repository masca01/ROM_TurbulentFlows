...
REF = dict(ds="Re100", n=250, lat=15, gen="galerkin_ns", lv=0.99, frac=0.5, noise=0.0,
           ridge=RIDGE, kick=IC_NOISE, horiz=GEN_TIME, smode="blocks", nsub=N_SUBSETS)
CSV_OUT = os.path.join(paths.RESULTS, "experiments2_results.csv")
# ================================
...
def _args(argv):
    o = {"plan": False, "exp": None, "subsets": None, "epochs": EPOCHS, "csv": CSV_OUT}
    it = iter(argv)
    for a in it:
...
def main():
    ...
    samples = sum(n for _, n, _, _, _ in bases) + sum(c["n"] + n_aug_for(c["frac"], c["n"]) for c, _ in todo)
    ...
    print(f"[exp2]  to do: {len(bases)} real-only + {len(todo)} augmented trainings, about "
          f"{samples * SEC_PER_SAMPLE * o['epochs'] / 500 / 3600:.1f} h of training  ->  {out}", flush=True)
    ...
    for ds, groups in by_ds.items():
        ...
        for (n, smode, sub), cs_ in groups.items():
            ...
            win_starts = fidelity_windows(set(int(i) for i in tr_idx), Nt, steps_q, rng)
            ...
            for lat in sorted({c["lat"] for c in cs_}):                       # real-only baselines first
                ...
                ek, detR, _, wall = vae.train(train_real, np.empty((0,) + train_real.shape[1:], np.float32), val_real,
                                              tag=f"{ds} n={n} {smode} s{sub} latent {lat} real", latent=lat, epochs=o["epochs"])
                    ...
                    try:
                        if gen in ("jitter", "real_proj", "real_full"):
                            ...
                            if gen == "jitter":
                                ...
                                # real snapshots the subset does not use, capped at what the pool holds
                                aug, take = real_arm(P, K, data, pool_idx, tr_idx, n_max, grng,
                                                     project=(gen == "real_proj"), cap=True)
                                if take < n_max:
                            ...
                            step = gns.make_step_ns(l0, q0, s, dt, SUBSTEPS)
                        ...
                        if gen in ("galerkin_ns", "galerkin"):
                            qerr, qblow = (fidelity(step, to_b, from_b, win_A, steps_q) if win_A else (None, ""))
                            if qerr is not None:
                                info.update(quality_err=round(qerr, 5), quality_blowup_frac=round(qblow, 3))
                            gseed = [DRAW_SEED, int(re_), n, sub, K, GIDX[gen]]
                            if abs(kick - IC_NOISE) > 1e-12 or abs(horiz - GEN_TIME) > 1e-12:
                                gseed += [int(round(kick * 1000)), int(round(horiz * 10))]
                            aug, st = synthetic_arm(step, s, to_b, from_b, P, A, K, n_max, steps_g,
                                                    np.random.default_rng(gseed),
                                                    kick=kick, energy_tol=ENERGY_TOL, max_traj=MAX_TRAJ)
                        ...
                        continue
                    for c in vcs:
                        na = min(n_aug_for(c["frac"], n), len(aug))   # capped for real_proj/real_full
