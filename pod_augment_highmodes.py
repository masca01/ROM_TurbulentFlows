"""
POD data augmentation — STRATEGY B: Perturb only the higher (small-scale) modes.
...
N_LEAD     = 2        # leading coherent modes kept from a real snapshot
STRATEGY   = "highmodes"
# ═══════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    # ── STRATEGY B: keep leading modes from a real snapshot, perturb the rest ──
    a_mean = A.mean(axis=0)
    a_std  = A.std(axis=0)
    base_t = rng.integers(0, Ntr, size=n_aug)       # random base snapshots
    A_new = A[base_t].copy()                          # start from real coeffs
    # resample only the higher modes
    A_new[:, n_lead:] = rng.normal(
        a_mean[None, n_lead:], a_std[None, n_lead:], size=(n_aug, r - n_lead))
    ...
    sio.savemat(out_file, {
        ...
        "n_modes": r, "n_lead": n_lead, "rank": (-1 if RANK is None else RANK),
    }, do_compression=True)

    print(f"\n[{STRATEGY}]  {base}  |  {H}x{W}  C={C}")
    print(f"  real train = {Ntr}   real val = {len(val_idx)}   synthetic = {n_aug}")
    print(f"  POD modes  = {r}   leading kept = {n_lead}   perturbed = {r - n_lead}")
